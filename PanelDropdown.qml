...
// Dropdown for a KeyboardPanel. Copied from crmne.hyprmoncfg (MIT, © 2026
// Carmine Paolino): the menu is positioned in panel coordinates instead of
// relying on Popup's implicit parent, which keeps it attached to the trigger
// when the panel is anchored away from the screen origin. It never assigns
// `value` itself; the caller updates its binding from `changed`.
Item {
  id: root
